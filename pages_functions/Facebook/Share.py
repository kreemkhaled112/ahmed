...
from pages_functions.Facebook.Data.Share import Share as share
            ...
            while  self.is_running :
                    ...
                    if pending["status"] == "success":
                        ...
                        if result[0] == 'No match found' :
                            self.Info.Add(0,id,'None',"Share",f'Error {result[1]}')
                                ...
                                while not self.queue.empty() :
                                    if self.succes >= quantity : break
                                        ...
                                        try:
                                            ...
                                            self.Info.Add(result[1],id,cookie[1],"Share",f'{result[0]} {link}')
                                            if result[1] == 1: self.succes += 1
                                            else: self.failed += 1
                                            self.Info.Update(s=self.succes,f=self.failed,o=self.order) ;  sleep(self.time)
                                        except Exception as e:
                                            try: self.Info.Add(0,id,cookie[1],"Share",e)
                                            except : self.Info.Add(0,id,'None',"Share","No Account") 
                                            self.failed += 1
                                            self.Info.Update(s=self.succes,f=self.failed,o=self.order) ;  sleep(self.time)
                               
                            except Exception as e : input(e)
                        self.order += 1
                        self.Info.Add(1,id,'Compelet',"Share",f'Succes : {self.succes} Failed : {self.failed}') ; self.Info.Update(s=self.succes,f=self.failed,o=self.order) 
